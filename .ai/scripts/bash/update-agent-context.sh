...
OPENCODE_FILE="$REPO_ROOT/OPENCODE.md"
...
# Template file
TEMPLATE_FILE="$REPO_ROOT/.ai/2_templates/agent-file-template.md"
...
update_specific_agent() {
    ...
    case "$agent_type" in
        ...
        opencode)
            ...
            ;;
        codex)
            update_agent_file "$OPENCODE_FILE" "Codex CLI"
            ;;
        windsurf)
...
update_all_existing_agents() {
    ...
    if [[ -f "$OPENCODE_FILE" ]]; then
        update_agent_file "$OPENCODE_FILE" "Codex/opencode"
        found_agent=true
    fi
