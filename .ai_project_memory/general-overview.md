...
2. **Event Management** - Per-event record with: name, **event type** (tipo de evento), **client**, start/end datetime, rich-text details and notes, **services** (servicios) offered, **providers/staff** (prestadores/empleados) assigned, **complimentary services** (bonificados), and an auto-computed financial summary (Costo total, Ganancia total, Precio sin bonificaciones, Total bonificado, Subtotal, Precio total). **The event price is never entered by hand: `totalPrice` is always derived as the subtotal of the service lines minus bonificados (`servicePrice − totalBonificado`). Providers and staff are venue costs that lower profit but never raise the price. There is no manual price field.** Events split into **Mis eventos** (sold) and **Presupuestados** (quotes); actions are **Presupuestar** (quote) and **Reservar** (reserve).
...
5. **Providers / Staff (Prestadores)** - Employees/providers assignable to events. Each provider has a catalog cost, but the per-event cost is **always explicit** (`EventProvider.cost`, never null): on assign it is snapshotted from the catalog and prefilled in the picker, and the user can override it (e.g. catering whose cost varies). A provider can be created at cost 0 and priced per event. The per-event cost feeds the event's `Costo prestadores`/ganancia and the *Pago prestadores* report (how much the venue owes them). Catalog-cost changes do not move existing events (snapshot).
