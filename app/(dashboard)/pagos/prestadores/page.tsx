...
export default async function PagosPrestadoresPage({ searchParams }: Props) {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">Pago a prestadores</h1>
      ...
      <form method="GET" className="flex flex-wrap items-end gap-3 p-4 border rounded-lg bg-muted/20">
        ...
        <div className="space-y-1">
          ...
          <select name="providerId" defaultValue={params.providerId ?? ""} className="border rounded px-2 py-1.5 text-sm">
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="border-b text-left bg-muted/40">
                <th className="py-2 px-3">Prestador</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-b hover:bg-muted/40">
                  ...
                  <td className="py-2 px-3 text-right font-medium">{fmt(r.provider.cost)}</td>
